...
export default function TablaDisparos() {
  ...
  const getUniqueValues = (col) => {
    const vals = disparos.map(d => {
      if (col === 'ID') return d.id || '';
      if (col === 'MAQUINA') return d.MAQUINA_CORTE || d.MAQUINA || '';
      if (col === 'RUTA') return d.RUTA_CORTE || d.RUTA || '';
      if (col === 'VOLUMEN') return d.VOLUMEN_DIARIO || d.VOLUMEN || '';
      if (col === 'MAX') return d.MAXIMO || d.MAX || '';
      if (col === 'MIN') return d.MINIMO || d.MIN || '';
      if (col === 'PZAS') return d.PIEZAS_RESTANTES || d.PZAS || '';
      return d[col] || '';
    });
          ...
          {filteredDisparos.map(d => {
            ...
            return (
              <tr key={d.id} style={{background: selected.includes(d.id) ? '#e3eafc' : undefined}}>
                ...
                <td style={{whiteSpace: 'normal', wordBreak: 'break-word', maxWidth: 120, background: estatusColor, color: estatusColor ? (estatusColor === '#ffd600' ? '#333' : '#fff') : undefined, fontWeight: 'bold', textAlign: 'center'}}>{d.ESTATUS}</td>
              </tr>
            );
          })}
        </tbody>
